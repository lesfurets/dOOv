...
public interface FieldModel extends Iterable<Entry<FieldId, Object>>, DslModel, StringMapper {
    ...
    default void computeFields(Collection<FieldId> fieldIds) {
        stream()
                .filter(entry -> fieldIds.contains(entry.getKey()))
                .forEach(entry -> computeField(entry.getKey(), entry.getValue()));
    }

    default void computeField(FieldId fieldId, Object value) {
